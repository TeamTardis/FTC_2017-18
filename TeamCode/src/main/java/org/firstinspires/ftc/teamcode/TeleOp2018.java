...
public class TeleOp2018 extends BaseIO_TardisRelicRecovery {
 ...
 */
    ...
    double wristPosition = .45; //Variable for wristPosition
    ...
    public void loop() {  //Start of main loop
        ...
        */
        ...
        if (gamepad2.right_trigger != 0) { //If right trigger is pressed, close claw
            s2.setPosition(0.8); //Sets servo position to 0.8
        ...
        if (gamepad2.x) { //If button x is pressed, extend wrist
            ...
            s4.setPosition(0.5); //Sets servo position to 0.5
        }
...
//            if (RLR == 0) {
                ...
                m1.setPower(((LRL + LUD) / 4) + (RLR / 6) - turn); //Turning for topm1.setPower(((LRL + LUD) / 8) + (RLR / 6) - turn); //Steering for top left
                m2.setPower(((LUD - LRL) / 4) - (RLR / 6) + turn); //Steering for top right
                m3.setPower(((LUD - LRL) / 4) + (RLR / 6) - turn); //Steering for back left
                m4.setPower(((LRL + LUD) / 4) - (RLR / 6) + turn); //Steering for back right
/*            } else { left motor
