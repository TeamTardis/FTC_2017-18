...
public class CalibrateColorSensor extends OpMode {
    ...
    private ModernRoboticsI2cColorSensor c1;
    ...
    public void init() {
        ...
        c1 = hardwareMap.get(ModernRoboticsI2cColorSensor.class, "c1");
    }
    ...
    public void loop() {
         ...
         */
        command = c1.readUnsignedByte(Register.COMMAND);

        if (!(command == 0 || command == 1)) {
            // Something is in progress. Do nothing.
        }
        else if (gamepad1.x || gamepad2.x) {
            c1.writeCommand(Command.CALIBRATE_BLACK);
        }
        else if (gamepad1.y || gamepad2.y) {
            c1.writeCommand(Command.CALIBRATE_WHITE);
        }
